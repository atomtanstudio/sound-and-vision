...
Suno was inspected directly in the user's signed-in internal-browser session on 10 September, using Simple, Advanced and Grid. No songs were generated, edited, published or downloaded there. Its rail / form / results relationship informed the information architecture. Sound and Vision uses original code, artwork, typography, surfaces and a much smaller navigation set. Reference screenshots in `reference/` are research materials, not shipped product assets.
