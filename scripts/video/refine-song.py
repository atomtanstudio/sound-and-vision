...
root = args.root
os.environ.setdefault("TORCH_HOME", str(root / "alignment-models/torch"))
...
old = json.loads(
    (root / "data/video" / args.alignment / "result.json").read_text()
)
heard = json.loads(
    (
        root / "data/video" / args.alignment / "transcription.json"
    ).read_text()
)
groups = []
current = None
...
audio = root / "data/runs" / args.take / "attempt-001/audio.flac"
vocals = (
    root
